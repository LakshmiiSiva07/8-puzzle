...
# The following code solves a 8-puzzle using A* Search with 3 heuristics
      ...
      if count < 1000:		   # Limit the search by setting the depth level by 1000
         ...
         if issolved(x):
          ...
          for every_node in successor_states:
           ...
           fval = hval + every_node.depth                      # The evalution function used for A* search algorithm
           ...
           if idx_closed == -1 and idx_open == -1:             # If node is neither in open list or closed list add it to
              ...
              if fval < copy.hval + copy.depth:
                ...
                if fval < copy.hval + copy.depth:          # If it is in the closed with a lesser heuristic value add it to open list
                   copy.hval = hval
                   closedlist.remove(copy)
                   openlist.append(every_node)     
          closelist.append(x)
          openlist = sorted(openlist, key=lambda p: p.hval + p.depth)  
      else:
        return [0,0] 
